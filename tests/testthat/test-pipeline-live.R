# ==============================================================================
# LIVE Pipeline Tests for sdschooldata
# ==============================================================================
#
...
test_that("South Dakota DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add South Dakota DOE URL and verify HTTP 200")
})
...
test_that("Can download South Dakota enrollment data file", {
  skip_if_offline()
...
test_that("Can parse South Dakota enrollment file", {
  skip_if_offline()
...
test_that("sdschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- sdschooldata::get_available_years()
  ...
  tryCatch({
    raw <- sdschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- sdschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- sdschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- sdschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- sdschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- sdschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- sdschooldata:::get_cache_path(2024, "enrollment")
